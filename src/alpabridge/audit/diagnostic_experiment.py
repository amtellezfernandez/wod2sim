...
from alpabridge.challenge.e2e_driver import AlpaBridgeChallengeAdapter
...
def run_diagnostic_experiment(
    ...
    *,
    ...
    accepted_schemas = {CURRENT_TELEMETRY_SCHEMA, "wod2sim_challenge_telemetry_v3"}
    if not accepted_schemas.issuperset(source_runtime["telemetry_schemas"]):
    ...
    wod_correct = sum(row["alpabridge_classification_correct"] for row in case_results)
    ...
    wod_detected = sum(row["alpabridge_fault_detected"] for row in fault_rows)
    wod_localized = sum(row["alpabridge_localization_correct"] for row in fault_rows)
    ...
    wod_false_positives = sum(row["alpabridge_fault_detected"] for row in control_rows)
    status_false_positives = sum(row["status_only_fault_detected"] for row in control_rows)
    wod_only_correct = sum(
        row["alpabridge_classification_correct"] and not row["status_only_classification_correct"]
        for row in case_results
    )
    status_only_correct = sum(
        row["status_only_classification_correct"] and not row["alpabridge_classification_correct"]
        for row in case_results
    )
    ...
    return {
        ...
        "classification": {
            "alpabridge": _classification_summary(
                correct=wod_correct,
                total=total,
            ...
            "paired_comparison": {
                "alpabridge_only_correct": wod_only_correct,
...
def _evaluate_case(case: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "alpabridge_valid": wod_valid,
        "alpabridge_observed_codes": observed_codes,
        "alpabridge_fault_detected": bool(observed_codes),
        "alpabridge_classification_correct": wod_valid == expected_valid,
        "alpabridge_localization_correct": (
...
def _benchmark_trace_decisions(
    ...
    *,
    ...
    samples: dict[str, list[float]] = {"alpabridge": [], "status_only": []}
    ...
    for _ in range(iterations):
        ...
        for case in case_order:
            methods = ["alpabridge", "status_only"]
            rng.shuffle(methods)
            for method in methods:
                if method == "alpabridge":
                ...
                if method == "alpabridge" and case["expected_fault_code"]:
                    fault_case_samples.append(elapsed_us)

    wod_summary = _timing_summary(samples["alpabridge"])
...
def _benchmark_adapter_guard_path(
    *,
    ...
    return {
        "scope": (
            "in-process AlpaBridgeChallengeAdapter.drive_once_to_proto route-following "
...
def _adapter_benchmark_fixture(
    *,
    guarded: bool,
) -> tuple[AlpaBridgeChallengeAdapter, list[tuple[str, int]]]:
...
def _implementation_hashes() -> dict[str, str]:
    ...
    source_files = (
        ...
        Path(inspect.getsourcefile(AlpaBridgeChallengeAdapter) or "").resolve(),
