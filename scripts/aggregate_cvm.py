...
from alpabridge.experiments.waymax_contract_study import load_retained_study
from alpabridge.neutral.alpasim_metrics import load_alpasim_metrics
...
def main() -> int:
    ...
    waymax_contract_study = load_retained_study(
        args.output.parent.parent / "external" / "waymax_contract_study"
    )
    summary = _summary(
        rows=rows,
        failures=failures,
        ...
        waymax_contract_study=waymax_contract_study,
...
def _protocol_replay_summary(path: Path) -> dict[str, Any]:
    ...
    for arm_name, file_stem, expected_mode, expected_model, expected_codes in (
        ...
        if (
            ...
            or adapter.get("version_id") != f"alpabridge-challenge-{expected_model}"
        ):
...
def _diagnostic_experiment_summary(path: Path) -> dict[str, Any]:
    ...
    required_numbers = (
        ...
        "classification.alpabridge.classification_correct",
        "classification.alpabridge.faults_detected",
        "classification.alpabridge.faults_correctly_localized",
        "classification.alpabridge.false_positives",
...
def _summary(
    *,
    ...
    waymax_contract_study: dict[str, Any],
    created_at: str,
) -> dict[str, Any]:
...
def _hash_rows(
    ...
    *,
    ...
    waymax_contract_study: dict[str, Any] | None = None,
) -> str:
    evidence = {
        "rows": rows,
        ...
        "waymax_contract_study": waymax_contract_study or {},
    }
...
def _write_tables(output: Path, summary: dict[str, Any], rows: list[dict[str, str]]) -> None:
    ...
    waymax_study = summary.get("waymax_contract_study", {})
    waymax_behavior = (
        waymax_study.get("behavior", {}) if isinstance(waymax_study, dict) else {}
    )
    waymax_endpoint = (
        waymax_behavior.get("endpoint_difference_m", {})
        if isinstance(waymax_behavior, dict)
        else {}
    )
    waymax_route_endpoint = (
        waymax_endpoint.get("route_following", {})
        if isinstance(waymax_endpoint, dict)
        else {}
    )
    waymax_cv_endpoint = (
        waymax_endpoint.get("constant_velocity", {})
        if isinstance(waymax_endpoint, dict)
        else {}
    )
    waymax_attribution = (
        waymax_behavior.get("attribution", {})
        if isinstance(waymax_behavior, dict)
        else {}
    )
    ...
    diagnostic_alpabridge = (
        diagnostic_classification.get("alpabridge", {})
    ...
    (tables / "waymax_factorial.tex").write_text(
        "% generated by contract-validation aggregate; data_hash="
        + data_hash
        + "\n"
        + "\\begin{tabular}{lrrrr}\n"
        ...
        + "\\midrule\n"
        + "Route following & "
        + f"{_format_table_metric(waymax_route_endpoint.get('median'))} & "
        + f"{_summary_int(waymax_route_endpoint, 'changed_count')}/"
        + f"{_summary_int(waymax_behavior, 'comparison_eligible_scenarios')} & "
        + f"{_summary_int(waymax_attribution, 'route_following_full_route_clean')}/"
        + f"{_summary_int(waymax_behavior, 'comparison_eligible_scenarios')} & "
        + "0/"
        + f"{_summary_int(waymax_behavior, 'comparison_eligible_scenarios')} \\\\\n"
        + "Constant velocity & "
        + f"{_format_table_metric(waymax_cv_endpoint.get('median'))} & "
        + f"{_summary_int(waymax_cv_endpoint, 'changed_count')}/"
        + f"{_summary_int(waymax_behavior, 'comparison_eligible_scenarios')} & "
        + f"{_summary_int(waymax_attribution, 'constant_velocity_full_route_clean')}/"
        + f"{_summary_int(waymax_behavior, 'comparison_eligible_scenarios')} & "
        + f"{_summary_int(waymax_attribution, 'constant_velocity_command_proxy_clean')}/"
        + f"{_summary_int(waymax_behavior, 'comparison_eligible_scenarios')} \\\\\n"
        + "\\bottomrule\n\\end{tabular}\n",
        encoding="utf-8",
    )
    ...
    (tables / "paper_numbers.tex").write_text(
        ...
        + f"\\newcommand{{\\CVMWaymaxScenarioCount}}{{{_summary_int(waymax_behavior, 'scenario_count')}}}\n"
        + f"\\newcommand{{\\CVMWaymaxEligibleScenarios}}{{{_summary_int(waymax_behavior, 'comparison_eligible_scenarios')}}}\n"
        + f"\\newcommand{{\\CVMWaymaxRouteUnavailableScenarios}}{{{_summary_int(waymax_behavior, 'route_unavailable_scenarios')}}}\n"
        + f"\\newcommand{{\\CVMWaymaxClosedLoopSteps}}{{{_summary_int(waymax_behavior, 'closed_loop_steps_total')}}}\n"
        + f"\\newcommand{{\\CVMWaymaxFinitePlans}}{{{_summary_int(waymax_behavior, 'finite_trajectory_plans')}}}\n"
        + f"\\newcommand{{\\CVMWaymaxRouteEndpointChanged}}{{{_summary_int(waymax_route_endpoint, 'changed_count')}}}\n"
        + f"\\newcommand{{\\CVMWaymaxRouteEndpointMaterialChanged}}{{{_summary_int(waymax_route_endpoint, 'material_change_count')}}}\n"
        + f"\\newcommand{{\\CVMWaymaxNegativeControlInvariant}}{{{_summary_int(waymax_behavior, 'negative_control_invariant_scenarios')}}}\n"
        + f"\\newcommand{{\\CVMWaymaxRouteFullClean}}{{{_summary_int(waymax_attribution, 'route_following_full_route_clean')}}}\n"
        + f"\\newcommand{{\\CVMWaymaxRouteProxyRejected}}{{{_summary_int(waymax_attribution, 'route_following_command_proxy_semantic_fault')}}}\n"
        + f"\\newcommand{{\\CVMWaymaxCVFullClean}}{{{_summary_int(waymax_attribution, 'constant_velocity_full_route_clean')}}}\n"
        + f"\\newcommand{{\\CVMWaymaxCVProxyClean}}{{{_summary_int(waymax_attribution, 'constant_velocity_command_proxy_clean')}}}\n"
        + "\\newcommand{\\CVMWaymaxRouteEndpointMeanM}{"
        + _paper_waymax_metric(summary, "behavior.endpoint_difference_m.route_following.mean")
        + "}\n"
        + "\\newcommand{\\CVMWaymaxRouteEndpointMedianM}{"
        + _paper_waymax_metric(
            summary, "behavior.endpoint_difference_m.route_following.median"
        )
        + "}\n"
        + "\\newcommand{\\CVMWaymaxRouteEndpointMaxM}{"
        + _paper_waymax_metric(summary, "behavior.endpoint_difference_m.route_following.max")
        + "}\n"
        + "\\newcommand{\\CVMWaymaxCVEndpointMaxM}{"
        + _paper_waymax_metric(
            summary, "behavior.endpoint_difference_m.constant_velocity.max"
        )
        + "}\n"
        + "\\newcommand{\\CVMWaymaxInteractionMeanM}{"
        + _paper_waymax_metric(
            summary, "behavior.difference_in_differences_endpoint_m.mean"
        )
        + "}\n"
        + "\\newcommand{\\CVMWaymaxInteractionMedianM}{"
        + _paper_waymax_metric(
            summary, "behavior.difference_in_differences_endpoint_m.median"
        )
        + "}\n"
        ...
        + f"\\newcommand{{\\CVMWODDiagnosticCorrect}}{{{_summary_int(diagnostic_alpabridge, 'classification_correct')}}}\n"
        + f"\\newcommand{{\\CVMStatusDiagnosticCorrect}}{{{_summary_int(diagnostic_status, 'classification_correct')}}}\n"
        + f"\\newcommand{{\\CVMWODDiagnosticFaultDetected}}{{{_summary_int(diagnostic_alpabridge, 'faults_detected')}}}\n"
        + f"\\newcommand{{\\CVMWODDiagnosticLocalized}}{{{_summary_int(diagnostic_alpabridge, 'faults_correctly_localized')}}}\n"
        + f"\\newcommand{{\\CVMWODDiagnosticFalsePositives}}{{{_summary_int(diagnostic_alpabridge, 'false_positives')}}}\n"
...
def _paper_waymax_metric(summary: dict[str, Any], dotted_path: str) -> str:
    study = summary.get("waymax_contract_study")
    value = _nested_value(study, dotted_path)
    if not isinstance(value, (int, float)) or not math.isfinite(float(value)):
        return "n/a"
    return f"{float(value):.3f}"
