...
from alpabridge.audit.diagnostic_trace_generation import generate_protocol_trace
...
def _write_cases(path: Path, rows: list[dict[str, Any]]) -> None:
    fields = [
        ...
        "expected_fault_code",
        "alpabridge_valid",
        "alpabridge_observed_codes",
        "alpabridge_fault_detected",
        "alpabridge_classification_correct",
    ...
    with path.open("w", newline="", encoding="utf-8") as handle:
        ...
        for row in rows:
            serialized = dict(row)
            serialized["alpabridge_observed_codes"] = ";".join(serialized["alpabridge_observed_codes"])
