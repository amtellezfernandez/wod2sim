...
if [ ! -f "alpabridge.pdf" ]; then
  echo "Missing paper artifact: alpabridge.pdf" >&2
  exit 1
fi

TEST_FILE_COUNT="$(find tests -maxdepth 1 -type f -name 'test_*.py' | wc -l | tr -d '[:space:]')"
PDF_SIZE="$(wc -c < alpabridge.pdf | tr -d '[:space:]')"
ALPABRIDGE_TEST_FILE_COUNT="$TEST_FILE_COUNT" \
  ALPABRIDGE_PDF_SIZE="$PDF_SIZE" \
  perl -0pi -e '
    s/(PDF size at audit:\s*)\d+(\s+bytes\.)/$1$ENV{"ALPABRIDGE_PDF_SIZE"}$2/;
    s/(Test directory:\s*`tests`\s+with\s*)\d+(\s+top-level test files\.)/$1$ENV{"ALPABRIDGE_TEST_FILE_COUNT"}$2/;
  ' "$REPORT"
...
redact() {
  ...
  ALPABRIDGE_REDACT_ROOT="$ROOT" \
    ALPABRIDGE_REDACT_HOME="${HOME:-/home/user}" \
    ALPABRIDGE_REDACT_HOST="$host" \
    perl -pe '
      BEGIN {
        $root = quotemeta($ENV{"ALPABRIDGE_REDACT_ROOT"} // "");
        $home = quotemeta($ENV{"ALPABRIDGE_REDACT_HOME"} // "");
        $host = quotemeta($ENV{"ALPABRIDGE_REDACT_HOST"} // "");
      }
