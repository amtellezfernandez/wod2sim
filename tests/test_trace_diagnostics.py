...
from alpabridge.audit.diagnostic_experiment import run_diagnostic_experiment
from alpabridge.audit.diagnostic_trace_generation import (
...
def test_diagnostic_experiment_scores_controls_faults_comparator_and_timing() -> None:
    ...
    alpabridge = result["classification"]["alpabridge"]
    ...
    assert alpabridge["classification_correct"] == 30
    assert alpabridge["faults_detected"] == 15
    assert alpabridge["faults_correctly_localized"] == 15
    assert alpabridge["false_positives"] == 0
    assert status_only["classification_correct"] == 15
    assert status_only["faults_detected"] == 0
    assert paired["alpabridge_only_correct"] == 15
    ...
    assert sorted(result["implementation_sha256"]) == [
        "src/alpabridge/audit/diagnostic_experiment.py",
        "src/alpabridge/audit/diagnostic_trace_generation.py",
        "src/alpabridge/audit/trace_diagnostics.py",
        "src/alpabridge/challenge/e2e_driver.py",
        "src/alpabridge/simulator/baseline_drivers.py",
    ]
