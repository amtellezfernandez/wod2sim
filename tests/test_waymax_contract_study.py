...
import alpabridge.experiments.waymax_contract_study as study
from alpabridge.experiments.waymax_contract_study import (
    METRIC_NAMES,
...
@pytest.mark.skip(
    reason=(
        "artifacts/external/waymax_contract_study/results-summary.json pins an "
        "implementation_sha256 over src/wod2sim/... files from before the "
        "WOD2Sim->AlpaBridge package rename; the renamed src/alpabridge/... modules "
        "can never reproduce that hash, so this frozen evidence can no longer be "
        "revalidated bit-for-bit against the current source tree."
    )
)
