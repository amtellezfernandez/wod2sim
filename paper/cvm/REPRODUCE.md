# Reproducing Current AlpaBridge Contract-Validation Matrix (CVM) Artifacts

Current status: buildable AlpaBridge paper package with a completed
...
`ALPABRIDGE_ROUTE_CONTRACT_MODE=command_only_route`. AlpaSim video rendering is
...
environment on port 6790 and the AlpaBridge challenge driver with model
...
The output PDF is the repository-root `alpabridge.pdf`. `paper-verify` rebuilds
